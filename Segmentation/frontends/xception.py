...
#Modified from the following. Models with dropouts added.
...
@slim.add_arg_scope
def stack_blocks_dense_bayesian(net,
                       blocks,
                       first_pass,
                       batch_size,
                       output_stride=None,
                       outputs_collections=None,one_parameter=False):
  """Stacks Xception blocks with LBD.
  """
  # The current_stride variable keeps track of the effective stride of the
  # activations. This allows us to invoke atrous convolution whenever applying
  # the next residual unit would result in the activations having stride larger
    ...
    with tf.variable_scope(block.scope, 'block', [net]) as sc:
      for i, unit in enumerate(block.args):
        ...
        with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
          ...
          if block.scope == 'middle_flow/block1' and (i + 1)%4==0:
              
              # Add LBD here              
              u_samp = tf.get_variable("u", [batch_size,1,1,unit.get('depth_list',[728,728,728])[2]],trainable=False)
              
              if one_parameter==False:
                  p = tf.get_variable("p", [unit.get('depth_list',[728,728,728])[2]],initializer=tf.initializers.random_normal(mean=0.0,stddev=0.05), trainable=False)
              else:
                  p = tf.get_variable("p", [1],initializer=tf.initializers.random_normal(mean=0.0,stddev=0.05), trainable=False)

              u_samp = tf.cond(tf.equal(first_pass,True), lambda: drop_gen.sample([tf.shape(net)[0],1,1,tf.shape(net)[3]]), lambda: u_samp)
              prob = tf.sigmoid(p)
...
@slim.add_arg_scope
def stack_blocks_dense_concrete(net,
                       blocks,
                       output_stride=None,
                       outputs_collections=None, one_parameter=False):
  """Stacks Xception blocks with Concrete dropout.
  """
  # The current_stride variable keeps track of the effective stride of the
  # activations. This allows us to invoke atrous convolution whenever applying
  # the next residual unit would result in the activations having stride larger
      ...
      for i, unit in enumerate(block.args):
        ...
        with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
          ...
          if block.scope == 'middle_flow/block1' and (i + 1)%4==0:
                  ...
                  #p = tf.get_variable("p", [tf.shape(net)[3]],initializer=tf.initializers.random_normal(mean=0.0,stddev=0.05))
                  if one_parameter==False:
                      p = tf.get_variable("p", [unit.get('depth_list',[728,728,728])[2]],initializer=tf.initializers.random_normal(mean=0.0,stddev=0.05))
                  else:
                      p = tf.get_variable("p", [1],initializer=tf.initializers.random_normal(mean=0.0,stddev=0.05))

                  temper=0.67
                  #u_samp = drop_gen.sample([tf.shape(outputs)[0],1,1])
                  u_samp = drop_gen.sample([tf.shape(net)[0],1,1,tf.shape(net)[3]])
                  prob_exp = tf.expand_dims(tf.expand_dims(tf.expand_dims(tf.sigmoid(p),axis=0),axis=0),axis=0)
                  #mask = tf.cond(tf.equal(is_training,False),lambda:tf.cast(tf.distributions.Bernoulli(logits=p).sample([tf.shape(net)[0],1,1]),tf.float32),lambda: tf.sigmoid((1.0/temper) * (p - tf.log(u_samp+eps2) + tf.log(1.0-u_samp+eps2))))/(prob_exp)
...
@slim.add_arg_scope
def stack_blocks_dense_mc(net,
                       blocks,
                       output_stride=None,
                       outputs_collections=None):
  """Stacks Xception blocks with MC dropout.
  """
  # The current_stride variable keeps track of the effective stride of the
  # activations. This allows us to invoke atrous convolution whenever applying
  # the next residual unit would result in the activations having stride larger
    ...
    with tf.variable_scope(block.scope, 'block', [net]) as sc:
      for i, unit in enumerate(block.args):
        ...
        with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
          ...
          if block.scope == 'middle_flow/block1' and (i + 1)%4==0:
            mask = tf.cast(tf.distributions.Bernoulli(logits=0.0).sample([tf.shape(net)[0],1,1,tf.shape(net)[3]]),tf.float32)
            net = net * mask
  ...
  with tf.variable_scope(
    ...
    with slim.arg_scope([slim.conv2d,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        end_points['pool2'] = end_points[scope + '/entry_flow/block2/unit_1/xception_module/separable_conv2_pointwise']
...
def xception_bayesian(inputs,
             ...
             scope=None,
             one_parameter=False):
  """Generator for Xception model with LBD
  """
  with tf.variable_scope(
      scope, 'xception', [inputs], reuse=reuse) as sc:
    end_points_collection = sc.original_name_scope + 'end_points'
    with slim.arg_scope([slim.conv2d,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        # Extract features for entry_flow, middle_flow, and exit_flow.
        net = stack_blocks_dense_bayesian(net, blocks,first_pass,batch_size, output_stride,one_parameter=one_parameter)
...
def xception_concrete(inputs,
             ...
             scope=None,
             one_parameter=False):
  """Generator for Xception model with Concrete dropout
  """
  with tf.variable_scope(
      scope, 'xception', [inputs], reuse=reuse) as sc:
    end_points_collection = sc.original_name_scope + 'end_points'
    with slim.arg_scope([slim.conv2d,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        # Extract features for entry_flow, middle_flow, and exit_flow.
        net = stack_blocks_dense_concrete(net, blocks, output_stride,one_parameter=one_parameter)
...
def xception_mc(inputs,
             ...
             reuse=None,
             scope=None):
  """Generator for Xception model with MC dropout
  """
  with tf.variable_scope(
      scope, 'xception', [inputs], reuse=reuse) as sc:
    end_points_collection = sc.original_name_scope + 'end_points'
    with slim.arg_scope([slim.conv2d,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        # Extract features for entry_flow, middle_flow, and exit_flow.
        net = stack_blocks_dense_mc(net, blocks, output_stride)
...
def xception_65(inputs, first_pass,
                num_classes=None,
                is_training=True,
                global_pool=False,
                keep_prob=0.5,
...
def xception_65_bayesian(inputs, first_pass,
                batch_size,
                num_classes=None,
                is_training=True,
                global_pool=False,
                ...
                multi_grid=None,
                reuse=None,
                scope='xception_65_bayesian',
                one_parameter=False):
  """Xception-65 model with LBD."""
  blocks = [
      xception_block('entry_flow/block1',
                     depth_list=[128, 128, 128],
                     skip_connection_type='conv',
  ...
  return xception_bayesian(inputs,
                  blocks=blocks,
...
def xception_65_concrete(inputs,
                num_classes=None,
                is_training=True,
                global_pool=False,
                keep_prob=0.5,
                ...
                multi_grid=None,
                reuse=None,
                scope='xception_65_concrete',
                one_parameter=False):
  """Xception-65 model with Concrete dropout."""
  blocks = [
      xception_block('entry_flow/block1',
                     depth_list=[128, 128, 128],
                     skip_connection_type='conv',
  ...
  return xception_concrete(inputs,
                  blocks=blocks,
                  num_classes=num_classes,
                  is_training=is_training,
                  global_pool=global_pool,
...
def xception_65_mc(inputs,
                ...
                multi_grid=None,
                reuse=None,
                scope='xception_65_mc'):
  """Xception-65 model with MC dropout."""
  blocks = [
      xception_block('entry_flow/block1',
                     depth_list=[128, 128, 128],
                     skip_connection_type='conv',

...
parser.add_argument('--model', type=str, default="DeepLabV3_plus_bayesian", help='The model you are using. See model_builder.py for supported models')
parser.add_argument('--frontend', type=str, default="XceptionBayesian", help='The frontend you are using. See frontend_builder.py for supported models')
...
network, init_fn, frontend_scope = model_builder.build_model_bayesian(model_name=args.model, \
                                                                      frontend=args.frontend, net_input=net_input, first_pass=first_pass, batch_size=args.batch_size, \
                                                                      num_classes=num_classes, crop_width=args.crop_width, \
                                                                      crop_height=args.crop_height, is_training=False,pretrained_file= pretrained_file + ".ckpt")
...
for i in range(4):
    ...
    p_scope = dropout_scope + '/p'
    u_scope = dropout_scope + '/u'
...
trainer2=tf.train.RMSPropOptimizer(learning_rate=0.0001, decay=0.995)

f_d_expanded=tf.expand_dims(f_d,axis=1)
grads2=[tf.reduce_sum((tf.squeeze(u)-0.5)*f_d_expanded,axis=0) for u in u_all]

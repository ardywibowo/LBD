...
def batch_features_labels(features, labels, batch_size):
    ...
    for start in range(0, len(features), batch_size):
        ...
        feature_out = features[start:end]
        labels_out = labels[start:end]     

        yield feature_out, labels_out
...
# Testing
save_model_path = './image_classification/Concrete/Concrete'
batch_size = 64
n_samples = 10
top_n_predictions = 5
...
def test_model(tmpFeatures):
    ...
    with tf.Session(graph=loaded_graph) as sess:
        ...
        for train_feature_batch, train_label_batch in batch_features_labels(tmpFeatures, test_labels, batch_size):
            ...
            trues = np.argmax(train_label_batch)
        ...
        np.save('accuracy_concrete', test_batch_acc_total/test_batch_count)
        np.save('softmax_concrete', test_batch_softmax)
        np.save('entropy_concrete', test_batch_entropy)
